...
//  private GpComScanParameters m_ScanParameters = new GpComScanParameters();
      ...
      if (this.m_DeviceParameters != null)
      {
        ...
        if (retval == Common.ERROR_CODE.SUCCESS)
        {
          ...
          case PARALLEL:
            break;
          case USB:
            debug.d("GpComDevice", "Creating USB port...");
            this.m_Port = new USBPort(this.m_DeviceParameters);
            break;
          case ETHERNET:
            debug.d("GpComDevice", "Creating Ethernet port...");
            this.m_Port = new EthernetPort(this.m_DeviceParameters);
            break;
          case BLUETOOTH:
            debug.d("GpComDevice", "Creating Bluetooth port...");
          ...
          {
            debug.d("GpComDevice", "Port creation successful");
            ...
            if (retval == Common.ERROR_CODE.SUCCESS)
            {
              ...
              if (retval == Common.ERROR_CODE.SUCCESS)
              {
                if ((this.m_Port.isPortOpen()) && (this.m_Port.getError() == Common.ERROR_CODE.SUCCESS))
                {
                  debug.d("GpComDevice", "Port is open");
                ...
                {
                  debug.d("GpComDevice", "Port is NOT open");
                }
              }
              else
              {
                debug.d("GpComDevice", "openPort returned: " + retval.toString());
              }
            }
          }
          else
          {
            debug.d("GpComDevice", "Port creation NOT successful");
  ...
  public Common.ERROR_CODE activateASB(Boolean drawer, Boolean onoffline, Boolean error, Boolean paper, Boolean slip, Boolean panelbutton)
  {
    ...
    debug.d("GpComDevice", "------- activateASB method -------");
    debug.d("GpComDevice", "m_Port==null? " + Boolean.toString(this.m_Port == null));
    ...
    Vector binaryData = GpTools.convertEscposToBinary(command);
    debug.d("GpComDevice", "binaryData==null? " + Boolean.toString(binaryData == null));
    debug.d("GpComDevice", "binaryData.size()=" + Integer.toString(binaryData.size()));

    retval = this.m_Port.writeData(binaryData);
    debug.d("GpComDevice", "m_Port.writeData returned: " + retval.toString());
