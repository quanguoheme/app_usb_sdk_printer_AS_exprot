...
//  private ReentrantLock m_imageBufferLock = new ReentrantLock();
  ...
  private Vector<Byte> getData(Vector<Byte> source, ReentrantLock sourceLock)
  {
    ...
    if (source.size() > 0)
    {
      ...
      {
        debug.d("Port.getData", "Exception occured: " + e.getMessage());
      }
      finally
      {
        sourceLock.unlock();
  ...
  private byte[] getDataBytes(Vector<Byte> source, ReentrantLock sourceLock)
  {
    ...
    if (source.size() > 0)
    {
      ...
      {
        debug.d("Port.getDataBytes", "Exception occured: " + e.getMessage());
      }
      finally
      {
        sourceLock.unlock();
...
//        break;
    ...
    {
    case DEVICESTATUS:
      debug.d("Port", "m_RealtimeStatusReady=true");
...
//      break;
    case ASB:
      debug.d("Port", "ASB data ready");
      break;
    case GENERAL:
      debug.d("Port", "GENERAL data ready");
      break;
    }
